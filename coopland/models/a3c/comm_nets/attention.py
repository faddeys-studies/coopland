...
class CommCellAttention(BaseCommCell):
    def __init__(self, rnn_cell, signal_size):
        ...
        self._att_st1_size = 19
        self._attention_st1 = tf.keras.layers.TimeDistributed(
            tf.keras.layers.Dense(self._att_st1_size, activation=tf.nn.leaky_relu)
        )
        self._attention_st2 = tf.keras.layers.TimeDistributed(
    ...
    def build(self, input_shape):
        ...
        self._attention_st1.build((n_agents, n_others, 2 * self.signal_size))
        self._attention_st2.build(
            (n_agents, n_others, self._att_st1_size + 2 * self.signal_size)
        )
        self.built = True

    def _get_additional_layers(self):
        return [self._attention_st1, self._attention_st2]
